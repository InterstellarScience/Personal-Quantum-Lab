...
def run_transmissions(alice_bits, alice_bases, bob_bases, eve_intercepts, eve_bases, detected_photon_numbers, dark_count_flags):
    bob_bits = []

    for i in range(len(alice_bits)):
        # Checking if pulses arrived at Bob's station
        if detected_photon_numbers[i] == 0:
            if dark_count_flags[i]:
        ...
        bob_bits.append(simulate_single_qubit(qc))
...
def run_transmissions_fast(alice_bits, alice_bases, bob_bases, eve_intercepts, eve_bases, detected_photon_numbers, dark_count_flags):
    bob_bits = []
