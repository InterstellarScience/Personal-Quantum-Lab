...
bb84_simple_protocol.png
does it look correct?

2. Background & Theory
2. Background & Theory

The BB84 protocol is a quantum key distribution (QKD) protocol that allows Alice and Bob to generate a shared secret key. Alice first creates a random list of classical bits and randomly chooses either the computational (Z) basis or the diagonal (X) basis to encode the quantum states each bit.

The four possible quantum states are:
...
Bob does not know which bases Alice used and therefore randomly chooses either the (Z) or (X) basis to measure each qubit. If Alice and Bob use the same basis, Bob obtains Alice's classical bit with certainty in the ideal case. If they use different bases, Bob's measurement result is random.

After all qubits have been measured, Alice and Bob publicly compare their basis choices without revealing their bit values. They discard all positions where their bases did not match. The remaining bits form their sifted keys, which should be identical in the ideal protocol without noise or an eavesdropper.
3. Implementation

The implementation follows the BB84 protocol described above. Alice first generates a random list of classical bits and a random list of encoding bases, where 0 represents the (Z) basis and 1 represents the (X) basis.

Each Qiskit qubit initially starts in the state (\lvert 0\rangle). Depending on Alice's classical bit and chosen basis, the required gates are applied to prepare one of the four BB84 quantum states. Bob then independently generates his own random list of measurement bases. For a (Z)-basis measurement, the qubit is measured directly. For an (X)-basis measurement, a Hadamard gate is applied before the measurement.

The circuit is simulated once using the Qiskit Aer simulator, since each transmitted qubit produces only one measurement result in the BB84 protocol. Alice's and Bob's bases are then compared, and only the bits corresponding to matching bases are kept. These remaining bits form Alice's and Bob's sifted keys.

This first implementation represents the ideal BB84 protocol without an eavesdropper, noise, photon loss, error correction, or privacy amplification.

dont forget, table must also be in github form

### 2. Background & Theory

The BB84 protocol is a quantum key distribution (QKD) protocol that allows Alice and Bob to generate a shared secret key. Alice first creates a random list of classical bits and randomly chooses either the computational \(Z\) basis or the diagonal \(X\) basis to encode the quantum state of each bit.
...
and use my master thesis writing style please
