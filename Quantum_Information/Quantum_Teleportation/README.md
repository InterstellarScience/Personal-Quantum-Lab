...
- The unknown qubit $q_0$ is prepared using the universal single-qubit gate (U gate), defined as $U(\theta,\phi,\lamda)=R_{z}(\theta)R_{y}(\phi)R_{z}(\lamda)$. Alice and Bob also share an entangled Bell state, which is conventionally chosen as $$|\Phi^+\rangle$$, with Alice's half of the Bell pair being $q_1$ and Bob's half being $q_2$.
